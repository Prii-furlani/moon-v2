...
export function App() {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      if (parsedUser.role !== 'admin_master' && (!parsedUser.id || !parsedUser.usuarioId)) {
  ...
  const handleAddLancamento = async (novo: Lancamento) => {
    const item = { ...novo, usuarioId: user?.usuarioId || user?.id };
  ...
  const handleAddDespesa = async (nova: DespesaMensal) => {
    const item = { ...nova, usuarioId: user?.usuarioId || user?.id };
  ...
  const handleAddEstetica = (novo: RitualEstetica) => {
    setEstetica(prev => [...prev, { ...novo, usuarioId: user?.usuarioId || user?.id }]);
    addAuditLog(
  ...
  const handleAddPet = async (novo: Pet) => {
    const item = { ...novo, usuarioId: user?.usuarioId || user?.id, ativo: true };
    try {
    ...
    try {
      ...
      if (res?.status === 'success') {
        ...
        const novaDespesaPrevista: DespesaMensal = {
          id: `desp_ag_${itemComId.id}`,           descricao: "Despesa Pet", categoria: "Pets", diaVencimento: day, valorPrevisto: itemComId.valor, status: 'previsto', mes: yearMonth
        };
  ...
  const handleConcluirAgendaPet = async (item: ItemAgendaPet, valorPago: number, metodoPagamento: string) => {
    try {
      const res = await fetchApiData<{ status: string; message?: string }>('pets.php?action=concluir_agenda', {
        method: 'POST',
        body: JSON.stringify({ ...item, valorPago, metodoPagamento })
      });
      if (res?.status === 'success') {
        ...
        const novoLancamento: Lancamento = {
          id: `lan_${Date.now()}`,           descricao: "Gasto Pet", categoria: "Pets", tipo: "despesa", valor: valorPago, dia: day, recorrente: false, status: 'realizado', metodoPagamento
        };
        ...
        setDespesas(prev => {
          ...
          if (existingIdx !== -1) {
            ...
            const novaDespesaPago: DespesaMensal = {
              id: `desp_ag_${item.id}`,               descricao: "Despesa Pet", categoria: "Pets", diaVencimento: day, valorPrevisto: valorPago, valorPago: valorPago, dataPagamento: todayStr, metodoPagamento, status: 'pago', mes: yearMonth
            };
  ...
  const handleFinalizarCompraMensalPet = async (historico: HistoricoCompraPet) => {
    ...
    const historicoParaSalvar = { ...historico, usuarioId: user?.usuarioId || user?.id };
    try {
      ...
      if (res?.status === 'success') {
        ...
        const novoLancamento: Lancamento = {
          id: `lan_${Date.now()}`,           descricao: "Gasto Pet", categoria: "Pets", tipo: "despesa", valor: historico.totalPago, dia: new Date().getDate(), recorrente: true, status: 'realizado'
        };
        setLancamentos(prev => [novoLancamento, ...prev]);

        const novaDespesaCompra: DespesaMensal = {
          id: `desp_compra_${Date.now()}`,           descricao: "Despesa Pet", categoria: "Pets", diaVencimento: new Date().getDate(), valorPrevisto: historico.totalPago, valorPago: historico.totalPago,
  ...
  const handleRegistrarEmergenciaPet = async (emergencia: EmergenciaPet) => {
    const emergenciaParaSalvar = { ...emergencia, usuarioId: user?.usuarioId || user?.id };
    try {
      ...
      if (res?.status === 'success') {
        ...
        if (valorRestante > 0) {
          ...
          const novoLancamento: Lancamento = {
            id: `lan_emerg_${Date.now()}`,             descricao: "Gasto Pet", categoria: "Pets", tipo: "despesa", valor: valorRestante, dia: new Date().getDate(), recorrente: false, status: 'realizado', metodoPagamento: emergencia.metodoPagamentoRestante
          };
          setLancamentos(prev => [novoLancamento, ...prev]);

          const novaDespesa: DespesaMensal = {
            id: `desp_emerg_${Date.now()}`,             descricao: "Despesa Pet", categoria: "Pets", diaVencimento: new Date().getDate(), valorPrevisto: valorRestante, valorPago: valorRestante, dataPagamento: todayStr,
  ...
  const handlePagarFaturaCartao = async (cartaoId: string, valorFatura: number, valorPago: number, metodoPagamento: string) => {
    try {
      ...
      if (res?.status === 'success') {
        ...
        const novoLancamento: Lancamento = {
          id: `lan_${Date.now()}`, descricao: "Lançamento Automático",            categoria: 'Cartão de Crédito', tipo: 'despesa', valor: valorPago, dia: new Date().getDate(),
        ...
        const novaDespesa: DespesaMensal = {
          id: `desp_${Date.now()}`, descricao: 'Pagamento de Fatura do Cartão', categoria: 'Cartão de Crédito', diaVencimento: new Date().getDate(), valorPrevisto: valorFatura,
        ...
        if (saldoDevedor > 0) {
          ...
          const despesaRollover: DespesaMensal = {
            id: `desp_roll_${Date.now()}`, descricao: 'Rollover (Transbordo Fatura Anterior)', categoria: 'Cartão de Crédito', diaVencimento: new Date().getDate(), valorPrevisto: saldoDevedor + juros,
  ...
  const handleAddInadimplencia = (nova: Inadimplencia) => {
    setInadimplencias(prev => [...prev, { ...nova, usuarioId: user?.usuarioId || user?.id }]);
    addAuditLog(
